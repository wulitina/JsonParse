...
public class Tokenizer {
    ...
    private void tokenize() throws IOException {
        // Use do-while to handle empty files
        Token token;
        do {
            token = start();
            tokens.add(token);
    ...
    private Token readNumber() throws IOException {
        ...
        StringBuilder sb = new StringBuilder();
        if (ch == '-') {    // Handle negative numbers
            sb.append(ch);
            ch = charReader.next();
            if (ch == '0') {    // Handle -0.xxxx
                sb.append(ch);
    ...
    private String readFracAndExp() throws IOException {
        ...
        if (ch == '.') {
            ...
            if (isExp(ch)) {    // Handle scientific notation
                sb.append(ch);
                sb.append(readExp());
            } else {
                if (ch != (char) -1) {
    ...
    private String readExp() throws IOException {
        ...
        if (ch == '+' || ch == '-') {
            ...
            if (isDigit(ch)) {
                ...
                if (ch != (char) -1) {    // Read to the end, no need to backtrack
                    charReader.back();
                }
            } else {
